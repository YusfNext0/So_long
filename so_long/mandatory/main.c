...
/*   Updated: 2024/06/02 18:11:27 by ynachat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	moving_player(int key, t_vars *v)
{
	...
	if (key == 126 && !ft_strchr("1E", v->up))
		moving(v, key);
	else if (key == 125 && !ft_strchr("1E", v->down))
		moving(v, key);
	else if (key == 124 && !ft_strchr("1E", v->right))
		moving(v, key);
	else if (key == 123 && !ft_strchr("1E", v->left))
		moving(v, key);
	check_exit(v, key);
	return (1);
}
...
int	main(int ac, char *av[])
{
	...
	vars.win = mlx_new_window(vars.mlx, vars.w * 32, vars.h * 32, "./so_long");
	if (!vars.win)
		return (free_mat(vars.map), free(vars.mlx), 0);
